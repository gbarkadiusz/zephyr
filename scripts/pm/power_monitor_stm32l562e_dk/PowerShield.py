...
from scripts.pm.power_monitor_stm32l562e_dk.SerialHandler import *
from scripts.pm.power_monitor_stm32l562e_dk.PowerShieldConfig import *
from scripts.pm.power_monitor_stm32l562e_dk.PowerShieldData import *
from scripts.pm.power_monitor_stm32l562e_dk.UnityFunctions import *
from scripts.pm.power_monitor_stm32l562e_dk.AbstractPowerMonitor import *
...
class PowerShield(PowerMonitor):
    ...
    def test_communication(self):
        ...
        """
        if not self.handler.is_open():
            logging.error(f"Error: Connection is not open. Cannot send version command: {e}")
            return ""
        command = 'version'
    ...
    def set_format(self, data_format: str = PowerShieldConf.DataFormat.ASCII_DEC):
        ...
        if data_format not in [PowerShieldConf.DataFormat.ASCII_DEC, PowerShieldConf.DataFormat.BIN_HEXA]:
            print(f"Error: Invalid format '{data_format}'. Valid options are 'ascii_dec' or 'bin_hexa'.")
            return
    ...
    def set_frequency(self, frequency: str):
        ...
        if frequency not in [PowerShieldConf.SamplingFrequency.FREQ_100K, PowerShieldConf.SamplingFrequency.FREQ_50K, PowerShieldConf.SamplingFrequency.FREQ_20K,
                             PowerShieldConf.SamplingFrequency.FREQ_10K, PowerShieldConf.SamplingFrequency.FREQ_5K, PowerShieldConf.SamplingFrequency.FREQ_2K,
                             PowerShieldConf.SamplingFrequency.FREQ_1K, PowerShieldConf.SamplingFrequency.FREQ_500, PowerShieldConf.SamplingFrequency.FREQ_200,
                             PowerShieldConf.SamplingFrequency.FREQ_100, PowerShieldConf.SamplingFrequency.FREQ_50, PowerShieldConf.SamplingFrequency.FREQ_20,
                             PowerShieldConf.SamplingFrequency.FREQ_10, PowerShieldConf.SamplingFrequency.FREQ_5, PowerShieldConf.SamplingFrequency.FREQ_2,
                             PowerShieldConf.SamplingFrequency.FREQ_1]:
            logging.warning(f"Error: Invalid frequency '{frequency}'. Valid options are: 100k, 50k, 20k, 10k, 5k, 2k, 1k, 500, 200, 100, 50, 20, 10, 5, 2, 1.")
            return
    ...
    def set_func_mode(self, function_mode: str = PowerShieldConf.FunctionMode.HIGH):
        ...
        """
        # Validate the input format
        if function_mode not in [PowerShieldConf.FunctionMode.HIGH, PowerShieldConf.FunctionMode.OPTIM]:
            print(f"Error: Invalid format '{function_mode}'. Valid options are 'ascii_dec' or 'bin_hexa'.")
            return
    ...
    def handle_metadata_end(self):
        ...
        if len(metadata_bytes) < 2:
            print("Incomplete end of acquisition metadata reveived.") 
            return
    ...
    def start_measurement(self):
        ...
        raw_to_file_Thread = threading.Thread(target=self.raw_to_file , args=(self.power_shield_conf.output_file,))
    ...
    def measure(self, time: int, freq: str=None, reset: bool = False):
        ...
        self.set_acquire_time(UnityFunctions.convert_to_scientific_notation(time=_time, unit=self.power_shield_conf.acquisition_time_unit))
    ...
    def get_data(self, unit: str = PowerShieldConf.MeasureUnit.RAW_DATA):
        if self.acqComplete:
            # Open the CSV file
            with open(self.power_shield_conf.output_file, mode='r', newline='') as file:
                csv_reader = csv.reader(file)
                for row in csv_reader:
                    self.power_shield_data.data.append(row[0])
                if unit == PowerShieldConf.MeasureUnit.CURRENT_RMS:
                    self.power_shield_data.current_RMS = UnityFunctions.calculate_rms(self.power_shield_data.data)
                    ...
                    for data in self.power_shield_data.data:
                        self.power_shield_data.power += float(float(data)*float(_delta_time)*float(self.target_voltage))
